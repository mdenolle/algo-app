...
import { analyzeLegoPhoto, visionProvider, type LegoColor, type LegoShape, type VisionResult } from './src/vision';
...
const brickColors: Record<BrickColor, string> = { blue:C.blue, red:C.red, yellow:C.yellow, green:C.green, white:'#DDE7EF', black:'#28313B', gray:'#8795A3' };
const inventory: { color: BrickColor; shape: BrickShape; count: number; name: string }[] = [
  {color:'blue',shape:'brick',count:6,name:'2 × 4 brick'},
  {color:'red',shape:'short',count:8,name:'2 × 2 brick'},
  {color:'yellow',shape:'plate',count:6,name:'thin plate'},
  {color:'green',shape:'slope',count:4,name:'slope'},
  {color:'white',shape:'curve',count:3,name:'curved piece'},
  {color:'black',shape:'wheel',count:4,name:'wheel'},
  {color:'gray',shape:'axle',count:3,name:'axle'},
];
const builds = [
  { id:'rocket', level:'QUICK BUILD', badge:'#DCF8E9', badgeInk:'#177347', icon:'🚀', title:'Pocket Rocket', detail:'12 pieces · 5 min', steps:[
    {text:'Start with one blue 2 × 4 brick.',hint:'Place it longways. This is the rocket’s strong base.',color:'blue' as BrickColor,shape:'brick' as BrickShape,piece:'2 × 4'},
    {text:'Stack a red 2 × 2 brick right in the center.',hint:'Line up the middle studs before you press down.',color:'red' as BrickColor,shape:'short' as BrickShape,piece:'2 × 2'},
    {text:'Add two thin yellow plates as wings.',hint:'Make both sides match like a mirror.',color:'yellow' as BrickColor,shape:'plate' as BrickShape,piece:'× 2'},
    {text:'Click the white sloped nose on top. Ready for launch!',hint:'Point the smallest end toward the sky.',color:'white' as BrickColor,shape:'slope' as BrickShape,piece:'SLOPE'},
  ]},
  { id:'dragon', level:'CREATIVE BUILD', badge:'#FFF1C0', badgeInk:'#856300', icon:'🐉', title:'Tiny Brick Dragon', detail:'23 pieces · 12 min', steps:[
    {text:'Build a green sloped base for the dragon’s body.',hint:'The long green slope makes a great backbone.',color:'green' as BrickColor,shape:'slope' as BrickShape,piece:'SLOPE'},
    {text:'Add the blue brick neck and head.',hint:'A little tilt makes your dragon look curious.',color:'blue' as BrickColor,shape:'brick' as BrickShape,piece:'× 3'},
    {text:'Give your dragon two thin yellow plate wings.',hint:'Try to make the wings even on both sides.',color:'yellow' as BrickColor,shape:'plate' as BrickShape,piece:'× 2'},
    {text:'Finish with a curved red tail and feet.',hint:'Wide feet will help the dragon stand.',color:'red' as BrickColor,shape:'curve' as BrickShape,piece:'CURVE'},
  ]},
  { id:'city', level:'MEGA BUILD', badge:'#FFE0E0', badgeInk:'#A52626', icon:'🏙️', title:'Skyline City', detail:'34 pieces · 20 min', steps:[
    {text:'Make a wide blue brick foundation.',hint:'A wide base keeps tall buildings steady.',color:'blue' as BrickColor,shape:'brick' as BrickShape,piece:'× 4'},
    {text:'Build the red 2 × 2 tower on the left.',hint:'Press each brick firmly before adding the next.',color:'red' as BrickColor,shape:'short' as BrickShape,piece:'× 8'},
    {text:'Build a green sloped tower on the right.',hint:'Try a different height for a fun skyline.',color:'green' as BrickColor,shape:'slope' as BrickShape,piece:'× 4'},
    {text:'Add thin yellow plate windows and a curved white rooftop.',hint:'Your city can have any window pattern you like.',color:'yellow' as BrickColor,shape:'plate' as BrickShape,piece:'× 9'},
  ]},
];
...
export default function App() {
  ...
  return <SafeAreaView style={s.safe}><StatusBar style="dark"/><View style={s.app}><Header home={()=>go('home')}/>
    ...
    {screen==='ideas'&&<ScrollView contentContainerStyle={s.page}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.inventory}>{detectedPieces.map(item=><View key={`${item.color}-${item.shape}`} style={s.inventoryItem} accessibilityLabel={`${item.count} ${item.color} ${item.name}`}><Brick color={item.color} shape={item.shape}/><Text style={s.count}>{item.count}</Text></View>)}</ScrollView>
    ...
    {screen==='builder'&&<ScrollView contentContainerStyle={s.page}>
      ...
      <View style={s.topPanel}><View style={s.panelHead}><Text style={s.panelLabel}>EXACT PIECE</Text><Text style={s.panelCount}>{Math.max(0,12-step*3)} LEFT</Text></View><View style={s.needed}><Brick color={build.steps[step].color} shape={build.steps[step].shape} label={build.steps[step].piece}/></View></View>
    ...
    {screen==='creator'&&<ScrollView contentContainerStyle={s.page}>
      ...
      <View style={s.topPanel}><View style={s.panelHead}><Text style={s.panelLabel}>SHAPES &amp; COLORS</Text><Text style={s.panelCount}>{Math.max(0,(scanResult?.totalPieces??34)-placed)} LEFT</Text></View><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.palette}>{detectedPieces.map(item=><Pressable key={`${item.color}-${item.shape}`} accessibilityLabel={`${item.color} ${item.name}`} style={[s.palettePiece,selectedPiece.color===item.color&&selectedPiece.shape===item.shape&&s.selected]} onPress={()=>setSelectedPiece(item)}><Brick color={item.color} shape={item.shape}/><Text style={s.count}>{item.count}</Text></Pressable>)}</ScrollView></View>
    ...
    {screen==='book'&&<ScrollView contentContainerStyle={s.page}>
      ...
      <View style={s.bookViewer}>
        ...
        {bookPage===1&&<View style={s.bookPage}><Text style={s.pageOverline}>BEFORE YOU BUILD</Text><Text style={s.pageTitle}>Shapes and colors you’ll need</Text><View style={s.bookInventory}>{detectedPieces.map(item=><View key={`${item.color}-${item.shape}`} style={s.inventoryItem}><Brick color={item.color} shape={item.shape}/><Text style={s.count}>{item.count}</Text></View>)}</View><Text style={s.pageCopy}>Match the exact shape, size, and color before each step.</Text></View>}
