...
import { Alert, Image, Platform, Pressable, SafeAreaView, ScrollView, StyleSheet, Text, TextInput, View, type ViewStyle } from 'react-native';
...
function WorldView({url}:{url:string}) {
  if(Platform.OS==='web')return createElement('iframe',{src:url,style:{border:0,width:'100%',height:'100%',display:'block'},allow:'fullscreen'});
  return <WebView source={{uri:url}} style={{flex:1}} javaScriptEnabled domStorageEnabled allowsFullscreenVideo originWhitelist={['*']} mixedContentMode="always" setSupportMultipleWindows={false} startInLoadingState renderError={()=><View style={s.worldError}><Text style={s.worldErrorTitle}>Algo World is not reachable</Text><Text style={s.worldErrorText}>Start the laptop server (npm run web in algo-app) and stay on the same Wi-Fi. Trying: {url}</Text></View>}/>;
}
...
export default function App() {
  ...
  return <SafeAreaView style={s.safe}><StatusBar style="dark"/><View style={s.app}><Header home={()=>go('home')} voiceOn={voiceOn} toggleVoice={toggleVoice}/>
    {screen==='home'&&<ScrollView contentContainerStyle={s.page}>
      ...
      <Pressable style={s.worldCard} onPress={()=>go('world')}><Text style={s.worldEmoji}>🌍</Text><View style={{flex:1}}><Text style={s.designTitle}>Explore Algo World</Text><Text style={s.designDetail}>Walk on a round LEGO planet · prototype</Text></View><Text style={s.arrow}>→</Text></Pressable>
    </ScrollView>}

    {screen==='world'&&<View style={s.worldScreen}><View style={s.worldBar}><Back onPress={()=>go('home')} label="Home"/><Text style={s.pillText}>ALGO WORLD · PROTOTYPE</Text></View><WorldView url={worldUrl()}/></View>}
...
const s=StyleSheet.create({
  ...
  worldCard:{marginTop:16,padding:14,flexDirection:'row',alignItems:'center',gap:12,borderRadius:18,backgroundColor:'#E3F3FF',borderWidth:2,borderColor:'#BFE0FF'},worldEmoji:{fontSize:34},worldScreen:{flex:1},worldBar:{height:44,paddingHorizontal:18,flexDirection:'row',alignItems:'center',justifyContent:'space-between'},worldError:{flex:1,padding:24,justifyContent:'center',backgroundColor:C.paper},worldErrorTitle:{color:C.dark,fontSize:18,fontWeight:'900',marginBottom:8},worldErrorText:{color:C.gray,fontSize:13},
